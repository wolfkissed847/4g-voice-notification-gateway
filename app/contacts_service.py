...
# เบอร์ในประเทศไทย 10 หลักพอดี — ไม่รับรูปแบบสากล (+66…) เพราะระบบนี้โทรในประเทศอย่างเดียว
# บังคับความยาวตายตัวแทนช่วง 8-15 เพราะเบอร์ที่พิมพ์ตกหรือเกินหนึ่งตัวจะผ่านช่วงกว้างๆ ไปได้
# แล้วไปโผล่เป็นสายที่โทรไม่ติดตอนเกิดเหตุจริง ซึ่งเป็นเวลาที่แย่ที่สุดที่จะรู้ว่ากรอกผิด
_PHONE_OK = re.compile(r"^\d{10}$")


def normalize_phone_number(raw: str) -> str:
    """
    ตัดตัวคั่นที่คนพิมพ์ติดมา (ช่องว่าง ขีด วงเล็บ) แล้วตรวจว่าเหลือตัวเลข 10 หลักพอดี
    ...
    if not _PHONE_OK.match(cleaned):
        raise InvalidPhoneNumberError(
            f"เบอร์โทร '{raw}' ไม่ถูกต้อง — ต้องเป็นตัวเลข 10 หลักพอดี "
            "(พิมพ์ขีดหรือเว้นวรรคคั่นมาได้ ระบบตัดให้เอง) ห้ามมีตัวอักษรหรือขึ้นบรรทัดใหม่"
        )
    return cleaned
