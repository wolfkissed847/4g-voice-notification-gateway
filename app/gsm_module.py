...
class GSMModule:
    ...
    def _force_release_via_radio(self):
        """
        ทางออกสุดท้าย: ตัดสายด้วยการปิด-เปิดคลื่นวิทยุ เมื่อคำสั่งวางสายทุกตัวไม่ได้ผล

        AT+CFUN=4 คือปิดภาครับ-ส่งสัญญาณ (เทียบเท่าโหมดเครื่องบิน) สายที่ค้างอยู่จะหลุดทันที
        เพราะลิงก์กับเสาสัญญาณหายไป ไม่ได้ขึ้นกับว่าเฟิร์มแวร์รับคำสั่งวางสายตัวไหน
        แล้ว AT+CFUN=1 เปิดกลับ โมดูลจะ register เข้าเครือข่ายใหม่เอง

        ยอมแลกกับอะไร: ช่วง register ใหม่ (ราว 10-30 วินาที) โทรออกไม่ได้ และหน้าเว็บจะเห็น
        สัญญาณหายไปชั่วครู่ — แต่ดีกว่าปล่อยให้สายค้างถาวรจนคิวทั้งระบบตาย เพราะสายค้าง
        รอคนปลายทางกดวางอาจกินเวลาเป็นชั่วโมง หรือไม่มีใครกดเลยถ้าเขาไม่รู้ตัว
        """
        ...
        logger.error("เปิดคลื่นวิทยุกลับแล้วแต่ยังไม่ register เข้าเครือข่ายใน 30 วิ — งานถัดไปอาจโทรไม่ออก")
