...
# คำสั่งที่ถูกต้องคือ AT+CCMXPLAY เท่านั้น
#
# ⚠️ คู่มือบอกว่ารองรับ amr/wav/mp3/pcm แต่ทดสอบกับ A7670E จริง 22 ก.ย. 2569 พบว่า
# ไฟล์ .amr เล่นไม่ออกจริง: อัปโหลดครบ ตอบ OK ทุกขั้น ขึ้น +AUDIOSTATE ครบคู่ แต่เล่นจบ
# ใน 0.24 วิ จากเสียงยาว 8.93 วิ = ปลายสายได้ยินความเงียบ โดยไม่มี error ให้จับเลย
# ใช้ mp3 เท่านั้น (ทดสอบแล้วเล่นเต็มความยาวจริง) และถ้าจะเปลี่ยนฟอร์แมตเมื่อไหร่
# ต้องวัด "ความยาวที่เล่นจริง" เทียบไฟล์ต้นทางเสมอ ดูแค่ OK ไม่พอ
...
# หลวมกว่ากฎตอนบันทึก (contacts_service บังคับ 10 หลักพอดี) โดยตั้งใจ — ด่านนี้มีหน้าที่เดียว
# คือกันอักขระที่ทำให้บรรทัดเดียวกลายเป็นสองคำสั่ง AT ไม่ใช่บังคับรูปแบบเบอร์ซ้ำอีกรอบ
# ถ้าตั้งให้ตรงกันเป๊ะ เบอร์เก่าในฐานข้อมูลที่บันทึกไว้ก่อนเปลี่ยนกฎจะโทรออกไม่ได้เลย
...
class GSMModule:
    ...
    def prepare_audio(self, audio_file_path: str, on_progress=None):
        ...
        ทำไมต้องแยกจากการเล่น: เดิมขั้นนี้กินเวลา 15-20 วินาที (ส่งทีละ 256 byte หน่วง 50ms)
        ถ้าอัปโหลดหลังปลายสายรับแล้ว คนรับจะเจอความเงียบยาวจนอาจวางสายไปก่อน
        ตอนนี้เหลือไม่ถึงวินาที (ดู _try_cftranrx) แต่ยังคงแยกขั้นไว้เหมือนเดิม เพราะ
        ถึงจะเร็วแล้วก็ยังไม่ควรให้คนรับต้องรอ และ gTTS เองก็ยังใช้เวลาอีก ~0.3-0.9 วิ
    ...
    def _try_cftranrx(self, module_path: str, data: bytes, on_progress=None) -> bool:
        ...
        # แบ่งส่งเป็นก้อน แต่ไม่ต้องหน่วงเวลาแล้ว — วัดกับ A7670E จริง 22 ก.ย. 2569
        #
        # เดิมใช้ 256 byte + หน่วง 50ms ตาม AT Command Manual หัวข้อ 13.2.1 NOTE 2 ที่เขียนว่า
        # "If sending file fails, increase the delay time between each 256 byte reach to 50ms"
        # — แต่ประโยคนั้นคือวิธี "แก้เมื่อส่งล้มเหลว" ไม่ใช่ค่าที่ต้องใช้ตั้งแต่แรก
        # ตอนนั้นน่าจะกระโดดจาก "ไม่หน่วงเลย" ไป "50ms" โดยไม่ได้ลองค่ากลาง ผลคือ throughput
        # เหลือ 5.1KB/วิ ทั้งที่สายวิ่งได้ 11.5KB/วิ = เวลากว่าครึ่งหมดไปกับการนั่งรอ sleep เฉยๆ
        #
        # วัดจริงด้วยไฟล์ 13KB: 256B+50ms ใช้ 2.72 วิ / 8192B ไม่หน่วง ใช้ 0.18 วิ (เร็วขึ้น 15 เท่า)
        # ทดสอบซ้ำ 6 รอบผ่านหมด ยืนยันขนาดไฟล์บนโมดูลด้วย AT+FSATTRI ว่าตรงกับต้นทางทุกรอบ
        #
        # ที่ยังต้องแบ่งก้อนอยู่ (ไม่ใช่ write ทีเดียวจบ) เพราะ pyserial จะ block จนกว่าจะเขียน
        # ลง OS buffer หมด การแบ่งก้อนทำให้รายงาน progress ระหว่างทางได้ และถ้าอนาคตเจอโมดูล
        # ที่รับไม่ทันจริง มีจุดให้ใส่ delay กลับเข้ามาได้โดยไม่ต้องรื้อโครงสร้าง
        #
        # หมายเหตุ: ลองเร่ง baud เป็น 460800 แล้วได้ 0.18 วิ เท่าเดิม — คอขวดไม่ใช่สายอีกต่อไป
        # จึงไม่เร่ง baud เพราะได้ความเสี่ยง (สองฝั่งตั้งไม่ตรงกัน = คุยกันไม่รู้เรื่อง) โดยไม่ได้อะไร
        chunk_size = 8192
        chunk_delay = 0.0
        total = len(data)
        ...
        for i in range(0, total, chunk_size):
            ...
            # รายงานความคืบหน้าตามจำนวน byte ที่ส่งไปแล้ว — ตอนนี้ขั้นนี้จบในไม่ถึงวินาที
            # หน้าเว็บอาจเห็นแค่แว่บเดียว ซึ่งเป็นเรื่องดี (เดิมค้างอยู่ขั้นนี้ 15-20 วินาที)
            if on_progress is not None:
                on_progress(min(1.0, (i + chunk_size) / total))

        # เวลารอ OK: อิงเวลาส่งจริงที่เพิ่งวัดได้ บวก buffer ให้โมดูลเขียนไฟล์ลง flash เสร็จ
        #
        # เดิมคำนวณจาก (จำนวนก้อน × chunk_delay) ซึ่งพอเลิกหน่วงเวลาแล้วจะได้ 0 เสมอ
        # กลายเป็นเหลือแต่ค่าพื้น 10 วิ โดยไม่โตตามขนาดไฟล์อีกเลย — ตอนนี้ไฟล์เล็กจึงยังพอ
        # แต่ถ้าวันหน้าไฟล์ใหญ่ขึ้นจะ timeout ทั้งที่โมดูลยังเขียนไม่เสร็จ วัดเวลาจริงแทนจึงตรงกว่า
