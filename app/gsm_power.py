...
PWRKEY_RISE_SECONDS = 0.6  # เวลาดันขาขึ้นก่อนกด เพื่อให้เกิดขอบขาลง (ดู _press)
...
class GsmPower:
    ...
    def _press(self, gpio: int, seconds: float) -> None:
        """
        จำลองการกดปุ่ม PWRKEY — ต้องสร้าง "ขอบขาลง" ให้โมดูลเห็น ไม่ใช่แค่ทำให้ขาต่ำ

        ── ทำไมต้องดันขึ้นก่อน (ข้อที่ห้ามลบ) ──────────────────────────────────
        บอร์ดที่ใช้จริงมีขา PWRKEY ที่ถูกดึงต่ำค้างอยู่ตลอดเวลา (วัดแล้ว: ตั้ง pull-up
        ภายใน 50k ของ Pi ก็ยังอ่านได้ lo) การสั่ง "ขับ LOW" ตรงๆ แบบเดิมจึงไม่มีอะไร
        เปลี่ยนแปลงในสายตาโมดูลเลย เพราะมันต่ำอยู่ก่อนแล้ว

        ผลคือกดยังไงก็ไม่ติด — ทดสอบมาแล้วทั้ง 1.5 / 2 / 3 / 5 / 6 วินาที ไม่เคยสำเร็จ
        สักครั้ง จนกระทั่งบังเอิญยิงลำดับ pull-up แล้วปล่อยลง โมดูลบูตทันทีใน 4 วินาที
        และทำซ้ำได้จริงกับโมดูลที่ดับสนิท

        ลำดับที่ใช้:
            1. input + pull-up   ดันขาขึ้นเท่าที่ดันได้ = สร้างจุดเริ่มของขอบ
            2. output ขับ LOW    ตกลงมา = ขอบขาลงที่โมดูลรอ + ค้างไว้ตามเวลากด
            3. input ไม่มีแรงดึง  ปล่อยกลับ hi-Z

        finally สำคัญมาก: ถ้ากดค้างแล้วเกิด exception ระหว่างนั้นโดยไม่ปล่อย ขาจะค้าง
        อยู่ที่ LOW ตลอดไป = โมดูลถูกกดปุ่มค้าง
        """
        # 1) ดันขึ้นก่อน — ใช้ pull-up ภายในเท่านั้น ห้ามขับ HIGH เด็ดขาด
        #    ขา PWRKEY อ้างอิงแรงดันภายในโมดูลซึ่งอาจเป็น 1.8V การขับ 3.3V ใส่ทำให้ขาพัง
        #    pull-up 50k จ่ายกระแสได้แค่ระดับไมโครแอมป์ จึงปลอดภัยแม้ปลายทางเป็นกราวด์
        self._lg.gpio_claim_input(self._chip, gpio, self._lg.SET_PULL_UP)
        time.sleep(PWRKEY_RISE_SECONDS)

        # 2) ตกลงมาเป็น LOW = ขอบขาลง แล้วค้างไว้
