...
#
# ── ทำไมลดจาก 500 เหลือ 100 (23 ก.ย. 2569) ────────────────────────────────
# เลข 500 เดิมตั้งจากเหตุผลเรื่อง "เวลาอัปโหลด" ซึ่งไม่ใช่ข้อจำกัดจริง
# ข้อจำกัดจริงคือ AT+CFTRANRX ของโมดูลรับไฟล์ได้ไม่เกิน 20,480 ไบต์พอดี (วัดทีละไบต์แล้ว)
# เกินแล้วตอบ ERROR = อัปไม่ได้ = โทรไม่ออกทั้งสาย ไม่ใช่แค่ช้าลง
#
# ⚠️ ต้องวัดด้วย "ข้อความหนาแน่นที่สุด" เสมอ (คำสั้นๆ ล้วน = จุลภาคถี่ = ช่วงเงียบเยอะ)
# เพราะจำนวนตัวอักษรทำนายขนาดไฟล์ไม่ได้แม่น — ข้อความปกติ 150 ตัวอักษรได้ 20,016 ไบต์
# แต่ข้อความหนาแน่น 150 ตัวอักษรได้ 27,504 ไบต์ ต่างกันเกือบ 40%
#
# วัดจริงกับข้อความไทยหนาแน่นสุดที่ 8kbps (บิตเรตเดียวที่ใช้แล้ว):
#     95 ตัวอักษร -> 18,360 ไบต์  เหลือที่ว่าง +2,120
#    100 ตัวอักษร -> 19,008 ไบต์  เหลือที่ว่าง +1,472  ← เลือกตรงนี้
#    105 ตัวอักษร -> 20,952 ไบต์  เกินเพดาน  -472
#
# เคยตั้ง 120 เมื่อ 22 ก.ย. ซึ่งวัดจากข้อความที่หนาแน่นไม่พอ — ลองซ้ำด้วยข้อความ
# หนาแน่นจริงแล้วได้ 22,032 ไบต์ = เกินเพดาน แปลว่า 120 ใช้ไม่ได้จริง จึงลดเหลือ 100
# ที่ยังเหลือที่ว่างพอให้ข้อความรูปแบบที่ยังไม่ได้ทดสอบไม่ทะลุเพดาน
MAX_MESSAGE_CHARS = 100
MAX_VARIABLE_VALUE_CHARS = 60
MAX_VARIABLES = 5
