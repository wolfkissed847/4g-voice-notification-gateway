...
export function CallLogPage() {
  ...
  /** ความสูงของ "แถวที่ไม่มีบรรทัดหมายเหตุ" ที่วัดได้ — ค่านี้คงที่ไม่ว่าหน้าไหน
   *  จำไว้เพื่อให้ตัวหารคงที่ตลอด (ดูหมายเหตุที่ effect คำนวณจำนวนแถว) */
  const plainRowH = useRef(0);
  /** fitTick ที่ค่า plainRowH ด้านบนวัดมา — ขนาดกล่อง/ฟอนต์เปลี่ยนเมื่อไหร่ต้องวัดใหม่ */
  const rowHTick = useRef(-1);
   ...
   *
   * ตอนนี้ตัวหารเป็นค่าคงที่ ไม่ใช่ค่าที่ขึ้นกับเนื้อหาของหน้า:
   * วัดจาก "แถวที่ไม่มีบรรทัดหมายเหตุ" เท่านั้น ซึ่งสูงเท่ากันทุกแถวทุกหน้า แล้วจำไว้
   * ต่อให้หน้าถัดไปมีหมายเหตุเยอะแค่ไหนก็หารด้วยค่าเดิม → คำนวณกี่รอบก็ได้คำตอบเดิม
   * = วนไม่ได้ตั้งแต่ต้น
   *
   * ── ทำไมเลิกใช้วิธี "วัดครั้งเดียวแล้วล็อก" ──
   * เคยแก้กระพริบด้วยการล็อกไม่ให้คำนวณซ้ำเลยหลังวัดครั้งแรก ซึ่งกันวนได้จริงแต่พังอีกทาง:
   * ถ้าจังหวะที่วัดครั้งแรกกล่องยังไม่เต็มความสูง (ฟอนต์ยังโหลดไม่เสร็จ / ผังยังไม่นิ่ง)
   * ค่าที่ผิดนั้นจะถูกล็อกไว้ตลอด ไม่มีอะไรมาแก้ให้ — อาการคือบางเครื่องได้ 8 แถว
   * ในกล่องที่ใส่ได้ 15 แล้วเหลือที่ว่างท้ายตารางค้างไว้ครึ่งกล่อง
   *
   * พอตัวหารคงที่แล้วก็ไม่ต้องล็อกอีก ปล่อยให้คำนวณใหม่ได้ทุกครั้งที่ rows เปลี่ยน
   * ค่าที่ผิดจากจังหวะแรกจึงแก้ตัวเองได้ในรอบถัดไป
   */
  useEffect(() => {
    const el = listRef.current;
    ...
    if (wrappers.length === 0) return; // ยังไม่มีแถวให้วัด รอรอบหน้า

    // ขนาดกล่อง/ฟอนต์เปลี่ยน = ความสูงแถวที่จำไว้ใช้ไม่ได้แล้ว
    if (rowHTick.current !== fitTick) {
      plainRowH.current = 0;
      rowHTick.current = fitTick;
    }
    const plain = wrappers.filter((w) => w.dataset.logRow === 'plain');
    if (plain.length > 0) plainRowH.current = Math.min(...plain.map((w) => w.offsetHeight));
    // ยังไม่เคยเจอแถวธรรมดาเลย (หน้านี้มีหมายเหตุทุกแถว) ค่อยใช้แถวที่เตี้ยที่สุดไปพลางก่อน
    const rowH = plainRowH.current || Math.min(...wrappers.map((w) => w.offsetHeight));
