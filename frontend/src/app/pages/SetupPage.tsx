...
export function SetupPage() {
  ...
  return (
    /* gap-3.5 ให้ตรงกับหน้าคิว/ประวัติ — เดิมเป็น gap-4 อยู่หน้าเดียว
       ...
       - กลุ่มผู้รับ ใช้ h-full เพราะข้างในมีกล่องที่ขอ "ที่ที่เหลือ" (flex-1) ไปเลื่อนในตัวเอง
         ซึ่งต้องการความสูงที่แน่นอนถึงจะคำนวณได้ min-h-full ใช้แทนไม่ได้ — min-height
         ไม่นับเป็นความสูงที่แน่นอนในการแบ่งพื้นที่ของ flex กล่องจะยืดตามเนื้อหาแล้วดันทั้งหน้า
         ให้เลื่อนแทนที่จะเลื่อนอยู่ในกล่อง (วัดจริงแล้ว: กล่องสูง 1011px แทนที่จะพอดีจอ)
       - อีกสองแท็บใช้ min-h-full เพราะยาวเกินจอได้ตามปกติ ถ้าตรึงเป็น h-full
         เนื้อหาส่วนที่เกินจะล้นออกไปโดยที่ padding ล่างของ main ไม่ทำงาน */
    <div className={cn('flex flex-col gap-3.5', tab === 'contacts' ? 'h-full min-h-0' : 'min-h-full')}>
