...
# แม่แบบชุดนี้ตั้งใจไม่ใส่ {ตัวแปร} เลย เพราะทุกตัวแปรต้องส่งมาจาก payload เท่านั้น
# (ดู event_types_service.render_message) แม่แบบที่มีตัวแปรจึงยิงผ่านได้เฉพาะเมื่อผู้ยิงส่งค่ามาครบ
# ซึ่งข้อมูลจำลองชุดนี้และปุ่มทดสอบในหน้าเว็บไม่ได้ส่งอะไรมาด้วย ระบบที่เพิ่งติดตั้งเสร็จ
# จึงต้องกดทดสอบแล้วได้ยินเสียงทันที ไม่ใช่เจอ 400 เป็นด่านแรก
#
# อยากให้พูดรายละเอียดเพิ่ม แก้แม่แบบให้มีตัวแปรแล้วส่งค่ามาด้วย เช่น
#   แม่แบบ  "แจ้งเตือน อุปกรณ์ {device} ที่ {location} อุณหภูมิ {temp} องศา"
#   payload {"event_type_code": "temp_high",
#            "variables": {"device": "ตู้ A", "location": "ชั้น 3", "temp": "48"}}
EVENT_TYPES: list[tuple[str, str, str]] = [
    ("node_down", "อุปกรณ์ขาดการเชื่อมต่อ", "แจ้งเตือน ตรวจพบอุปกรณ์ขาดการเชื่อมต่อ กรุณาตรวจสอบด่วน"),
    ("power_outage", "ไฟฟ้าดับ", "แจ้งเตือน ตรวจพบไฟฟ้าดับ กรุณาตรวจสอบด่วน"),
    ("temp_high", "อุณหภูมิสูงเกินกำหนด", "แจ้งเตือน ตรวจพบอุณหภูมิสูงเกินค่าที่ตั้งไว้ กรุณาตรวจสอบ"),
    ("water_leak", "ตรวจพบน้ำรั่ว", "แจ้งเตือน ตรวจพบน้ำรั่วในพื้นที่ กรุณาตรวจสอบด่วน"),
    ("smoke_detected", "ตรวจพบควัน", "แจ้งเตือนฉุกเฉิน ตรวจพบควัน กรุณาตรวจสอบทันที"),
    ("ups_low_battery", "แบตเตอรี่สำรองใกล้หมด", "แจ้งเตือน แบตเตอรี่สำรองเหลือน้อย กรุณาตรวจสอบ"),
